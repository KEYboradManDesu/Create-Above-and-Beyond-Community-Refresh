...
onEvent('item.registry', event => {
    ...
    trade("Exchange Currencies A", 0x9c4529, 0x9fadb4, [
		{ in: S(1), out: C(64) },
		{ in: C(64), out: S(1) },
	], true)

	trade("Exchange Currencies B", 0x9fadb4, 0xEBA83A, [
		{ in: G(1), out: S(64) },
		{ in: S(64), out: G(1) },
	], true)
	...
	profession("Farming", 0xFFCC29, 0x81B214, [
		...
		{ in: Q('apple_crate', 32), out: S(1) },
		{ in: Q('golden_apple_crate', 2), out: S(1) },
	...
	let simple = (name, item, coin, unit, c1, c2) => {
		...
		let template = `
		{
			title: "${amount}x {item.${item.replace(":", ".")}}"
			icon: "${id}"
			disable_toast: true
			x: ${x}d
			y: ${y}d
			shape: "hexagon"
			subtitle: "${coin} ${silver ? "'{item.thermal.silver_coin}'" : "'{item.thermal.gold_coin}'"}",
			tasks: [{
				type: "item"
	...
	while (true) {
		...
		simple("Weathered Limestone", CRD('weathered_limestone', 16), 3, S, 0x5E6055, 0x797E74)
		simple("Gabbro", CRD('gabbro', 16), 3, S, 0x6B5D4F, 0x7D6B5A)
		simple("Scoria", CR('scoria', 16), 3, S, 0x2A130C, 0x553427)
		simple("Scorchia", CR('scorchia', 16), 3, S, 0x1b1b1e, 0x55655a)
		...
		// simple("Aluminum Ingot", IM('ingot_aluminum', 8), 48, S, 0xD99413, 0xFFF5EE)
		...
		simple("Nebulous Heart", RQ('nebulous_heart', 1), 2, G, 0x6200A0, 0xE500C3)
